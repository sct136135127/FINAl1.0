...
public class NormalCreature extends SpecialCreature{
	public NormalCreature(String name,int x,int y,int num)
	{
		super(name,x,y);
		damage = 1;
		blood = 3;
		...
		stand = Main.Constant.soldierstand;
		attack = Main.Constant.soldierattack;
		beattack = Main.Constant.soldierbeattack;
		dead = Main.Constant.soldierdead;
	...
	public boolean attack()				//攻击范围 上下左右
	{
		...
		if(y>0&&judgeattack(x,y-1))
		{
			...
			{
				actionattack(x,y-1);
				//application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" attack "+x+" "+(y-1));
			}
			else
				attackflash();
		}
		...
		else if(x<11&&judgeattack(x+1,y))
		{
			...
			{
				actionattack(x+1,y);
				//application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" attack "+(x+1)+" "+y);
			}
			else
				attackflash();
		}
	...
	public void preparemove()						//准备移动
	{
		...
		{
			move(x-1,y);
			//application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" move "+(x-1)+" "+(y));
		}
		else if(y<9&&application.Main.space.space[x][y+1] == null&&(flag == 3||flag == 4))
		{
			move(x,y+1);
			//application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" move "+x+" "+(y+1));
		}
	...
	public void move(int x1,int y1) {			//移动
		//save
		application.Main.space.space[x][y] = null;
		application.Main.space.space[x1][y1] = this;
		...
		if(application.Main.saveflag == true)
			application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" move "+x+" "+y);
		//update
	};
	...
	{
		attackflash();
		Creature t = (Creature)application.Main.space.space[x][y];
		t.suffer(damage);
		
		if(application.Main.saveflag == true)
			application.Main.fc.recorder.add(this.getClass().getSimpleName()+" "+num+" attack "+x+" "+y);
	}
}
